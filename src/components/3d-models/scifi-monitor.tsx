...
const modelPath = "/assets/3d-models/scifi-monitor/scene.gltf"


export const ScifiMonitor = (props: GroupProps) => {
  const group = useRef()
  ...
  useFrame(() => {
    if (group.current) {
      group.current.rotation.x += 0.003; // Moderate speed
      group.current.rotation.y += 0.003
      group.current.rotation.z += 0.002; // Optional: Adjust Z separately
    }
  });

  useEffect(() => {
    if (actions) {
      const action = actions['Take 001']; // Make sure to use the exact name here
      if (action) {
